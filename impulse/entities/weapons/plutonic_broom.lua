-- "addons\\plutonic\\lua\\weapons\\plutonic_broom.lua"
...
if SERVER then
	...
	timer.Create("longswordBroomCleaner", 3, 0, function() -- this is very very very messy but i cba
		for v,k in pairs(PROP_BROOMS) do
			...
			if not IsValid(x) or x:Team() != TEAM_VORT or not IsValid(x:GetActiveWeapon()) or x:GetActiveWeapon():GetClass() != "plutonic_broom" then
